...
- `Services/`: Address resolution, command routing, profile-scoped local history, snapshot seeding, and SQLite-backed session persistence.
...
- Add durable local history persistence and history management UI with the same private-profile filtering guarantees.
