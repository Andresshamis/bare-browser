...
public struct URLSecurityPolicy: Sendable {
    ...
    public var allowedWebDownloadSchemes: Set<String>
    ...
    public init(
        ...
        allowedWebDownloadSchemes: Set<String> = ["http", "https", "blob", "data"],
        ...
        self.allowedWebDownloadSchemes = allowedWebDownloadSchemes
    ...
    public func decision(forWebDownloadURL url: URL) -> Decision {
        guard let scheme = url.scheme?.lowercased(), !scheme.isEmpty else {
            return .block(reason: "Missing URL scheme.")
        }

        if allowedWebDownloadSchemes.contains(scheme) {
            return .allowInWebView
        }
